...
When nothing follows, it stops explaining and starts suggesting: it picks the narrowest cell it can
and says **think about what happens if this were a 4**, choosing whichever guess falls over soonest
so you follow one or two forced moves to a wall rather than wandering. It works up from two options
...
Over **300 blank grids**, hints only: **300 finish**, none give up, **0 finish on a wrong grid**. So
every puzzle the generator makes can be talked through from a blank grid to the end, and no hint
along the way was ever wrong. An ordinary hint costs a fraction of a millisecond; the fallback,
which runs only when the whole cascade came back empty, about 4 ms.
...
Getting the last 3 was a matter of not giving up too early: the fallback only looked at cells with
two or three options, and on those boards none was refutable while dozens of refutations sat on
wider cells. It now widens until it finds one.
